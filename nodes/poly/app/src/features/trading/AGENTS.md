...
- **TENANT_FILTER_IN_EVERY_SNAPSHOT_QUERY** _(bug.5022)_ — every SQL read/write that touches `poly_copy_trade_{fills,decisions}` filters explicitly on `billing_account_id`. Even legacy root methods carry this filter (back-compatible bug fix). Lint-time backstop: `.dependency-cruiser.cjs` `tenant-scoped-tables-via-order-ledger-only` forbids raw `polyCopyTradeFills` / `polyCopyTradeDecisions` imports outside this slice (two named exceptions owned by task.5012).
- **RLS_BACKSTOP_TRACKED_IN_TASK_5012** — the runtime DB-layer backstop (`withTenantScope(appDb, ctx.created_by_user_id, ...)` around every `TenantOrderLedger` method) is intentionally NOT in this PR. Bundled with the privy-adapter migration in task.5012 Phase 1 so the `serviceDb` → `appDb` role switch lands once across both surfaces and is validated against a single canary. Until that lands, the explicit `billing_account_id` filter in every query is the structural floor.
...
- **DB client (v0):** uses `serviceDb` (BYPASSRLS). Per-tenant safety is the explicit `eq(billingAccountId, ...)` filter in every query (bug.5022 fix) plus the `forTenant(ctx)` envelope on the type layer plus the lint rule. The DB-layer RLS backstop (`withTenantScope(appDb, ...)`) lands in task.5012 Phase 1 — bundled with the privy-adapter migration so the role switch happens once.
