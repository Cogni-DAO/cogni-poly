...
- **FORTENANT_READS_THROUGH_RLS** _(bug.5022)_ — every read on the `TenantOrderLedger` returned by `OrderLedger.forTenant(ctx)` (`snapshotState`, `cumulativeIntentForMarketToken`, `hasOpenForMarket`, `findOpenForMarket`) runs inside `withTenantScope(appDb, ctx.created_by_user_id, ...)` — Postgres RLS on `poly_copy_trade_{fills,decisions}` is the runtime backstop. Even if a future query forgets the explicit `eq(billingAccountId, ...)` filter, the DB layer strips rows owned by another `created_by_user_id`. The two `TenantOrderLedger` writes (`insertPending`, `recordDecision`) still flow through `deps.db` (serviceDb) and stamp tenant attribution explicitly in the row values — not the bug.5022 leak surface; task.5012 migrates them.
...
- **DB clients:** `appDb` (RLS-enforced `app_user` role) for every `TenantOrderLedger` read — wrapped in `withTenantScope(appDb, ctx.created_by_user_id, ...)` so Postgres RLS becomes the structural floor. `serviceDb` (BYPASSRLS) for: (a) the legacy root snapshotState path (back-compat, carries explicit filter), (b) the two `TenantOrderLedger` writes (`insertPending`, `recordDecision`) until task.5012 migrates them, (c) the explicitly-named cross-tenant ops (`findStaleOpen`, `syncHealthSummary`, reconciler reads). Roles enumerated in `OrderLedgerDeps`.
