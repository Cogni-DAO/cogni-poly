...
- **CAP_IS_PER_CONDITION_ID** (bug.5054) — per-target `max_usdc_per_condition` is the cumulative-intent budget per Polymarket conditionId (one "bet question"); YES + NO outcomes of the same conditionId share the budget; different conditionIds (different lines on the same event) each get their own. DB column `mirror_max_usdc_per_trade` and Loki decision-log field `mirror_max_usdc_per_trade` retained for external compatibility. See spec invariant of the same name.
