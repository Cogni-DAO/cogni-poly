...
- **CI gotcha (sidecar-only PRs):** `verify-buildsha` probes the app's `/version` against the source-sha map. Catalog-v2 affected-only detection correctly skips the app rebuild when only `nodes/poly/sidecars/**` changes, but the map still records the PR head sha for `poly`, so the gate fails even though the sidecar promotion succeeded. Workaround: include a one-line app-side touch (this file qualifies) until the gate learns to verify per-image. Hit on bug.5005 Phase 1 (PR #73) and pre-emptively included on bug.5005 Phase 2 (PR #79) — same workaround.
