 ...
 * Purpose: Poly-local Doltgres adapter for work_items — implements the v0 CRUD surface (Query + create + patch + delete) for task.5044.
 * Scope: Reads/writes `work_items` in `knowledge_poly`. Auto-commits on every write per AUTO_COMMIT_ON_WRITE.
 * Invariants:
 ...
 *   - AUTO_COMMIT_ON_WRITE: Each create/patch/delete issues `dolt_commit('-Am', ...)` before returning.
 *   - AUTHOR_ATTRIBUTED: dolt_commit messages embed an `authorTag` derived from the request's session user.
 *   - ID_RANGE_RESERVED: Allocator floor is 5000 per type — matches operator's allocator so per-node and central views never collide on auto-allocated ids.
...
export class DoltgresPolyWorkItemAdapter implements WorkItemsDoltgresPort {
  ...
  async create(
    ...
    await this.sql.unsafe(
      `SELECT dolt_commit('-Am', ${escapeValue(`${COMMIT_TAG}: create ${allocatedId} by ${authorTag}`)})`
    );
  ...
  async patch(
    ...
    await this.sql.unsafe(
      `SELECT dolt_commit('-Am', ${escapeValue(`${COMMIT_TAG}: patch ${input.id as string} by ${authorTag}`)})`
    );
  ...
  async delete(id: WorkItemId, authorTag: string): Promise<boolean> {
    ...
    await this.sql.unsafe(
      `SELECT dolt_commit('-Am', ${escapeValue(`${COMMIT_TAG}: delete ${id as string} by ${authorTag}`)})`
    );
    return true;
  }
}
