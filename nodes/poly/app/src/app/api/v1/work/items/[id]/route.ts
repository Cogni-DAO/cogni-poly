 ...
 * Purpose: HTTP endpoints for getting, patching, and deleting a single poly work item by ID (task.5044).
 * Scope: Auth-protected GET (Doltgres-first, markdown fallback), PATCH (Doltgres only), DELETE (Doltgres hard-delete captured in dolt_log).
...
/**
 * GET /api/v1/work/items/:id — Get a single work item by ID. Doltgres-first;
 * markdown fallback only for unimported legacy ids.
 */
