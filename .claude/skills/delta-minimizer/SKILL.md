...
Accepts: event slug (`lal-bet-elc-2026-05-12`), conditionId (`0xb974…`), comma-separated conditionIds, or fuzzy event-title text. Produces a timestamped subdir at `nodes/poly/research/delta-minimizing/<slug>-<iso>/` containing:
...
The charter at `work/charters/POLY_COPY_DELTA.md` is the **source of truth** and is **LLM-owned**. The data tool (`poly-mirror-report.ts`) does NOT touch it; you do, surgically, in step 3 below. A separate one-shot renderer, `scripts/render-charter.ts`, refreshes the static `nodes/poly/research/delta-minimizing/charter.html` from the `.md`. Run it by hand any time you edit the charter — never auto, never as part of investigation. Reports are tracked in git (history is essential).
...
**After any charter edit, run** `pnpm tsx scripts/render-charter.ts` to refresh `nodes/poly/research/delta-minimizing/charter.html` — it's a viewable mirror of the `.md`, regenerated by hand, never by the data tool.
...
- Output dir: `nodes/poly/research/delta-minimizing/<slug>-<iso>/`
- Charter (LLM-owned, source of truth): `work/charters/POLY_COPY_DELTA.md`
- Charter (viewable mirror): `nodes/poly/research/delta-minimizing/charter.html`
