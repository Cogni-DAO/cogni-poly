...
6. **For per-node-AppSet repos** (`cogni-poly`): create deploy branches before first Argo sync. Use `gh api` (husky pre-push runs tests on every push):
   ```bash
   MAIN_SHA=$(git ls-remote origin refs/heads/main | awk '{print $1}')
   for ref in deploy/${env} deploy/${env}-poly; do
