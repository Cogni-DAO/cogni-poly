...
**Bootstrap first**: read `AGENTS.md`, the parent project (`work/projects/proj.*`), the governing spec(s), and `GET https://poly.cognidao.org/api/v1/work/items?node=<node>&projectId=proj.<x>` for adjacent items. Don't duplicate.

## API call

```bash
curl -X POST https://poly.cognidao.org/api/v1/work/items \
