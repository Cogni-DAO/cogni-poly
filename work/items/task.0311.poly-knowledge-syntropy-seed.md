...
This follows Dolt's own recommendation for single-branch users. Per [DoltHub's schema-migrations blog](https://www.dolthub.com/blog/2022-07-20-schema-migrations/): _"The same process outlined above can work in a Dolt database with a single branch."_ The "same process" is standard migration tooling (Flyway/Liquibase/drizzle-kit). Commit-history capture is handled by the [`@@dolt_transaction_commit` system variable](https://docs.dolthub.com/sql-reference/version-control/dolt-system-variables) — setting it to `1` auto-creates a Dolt commit for every transaction, so every drizzle-generated migration produces an auditable commit without application code changes.

One Dolt wrinkle: per [dolt#4843](https://github.com/dolthub/dolt/issues/4843), `CREATE`/`ALTER` statements do not honor autocommit. The migrator therefore wraps each migration run in:
...
The trailing `dolt_commit()` captures any DDL that autocommit missed. This three-step incantation is documented at `nodes/poly/app/schema/README.md` for future contributors.
