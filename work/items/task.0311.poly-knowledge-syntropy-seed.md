...
**Schema source-of-truth**: drizzle-kit. TS schema lives at `nodes/poly/app/schema/knowledge.ts` (re-exports from `@cogni/poly-knowledge/schema` → `@cogni/node-template-knowledge/schema`). Migrations generated by `drizzle-kit generate` against `drizzle.poly.doltgres.config.ts`.

This follows Dolt's own recommendation for single-branch users. Per [DoltHub's schema-migrations blog](https://www.dolthub.com/blog/2022-07-20-schema-migrations/): _"The same process outlined above can work in a Dolt database with a single branch."_ The "same process" is standard migration tooling (Flyway/Liquibase/drizzle-kit).

**Commit-history capture in v0**: the migrator runs each generated SQL file, then stamps a named Dolt commit via `SELECT dolt_commit('-Am', 'migration: <version>')`. The `-a` flag stages all working-set changes from the migration, so one Dolt commit per migration file captures the complete delta.

Dolt's [`@@dolt_transaction_commit` session variable](https://docs.dolthub.com/sql-reference/version-control/dolt-system-variables) can auto-commit every SQL `COMMIT` boundary — useful if you want finer-grained Dolt commits, but the `@@variable` MySQL syntax is not verified on Doltgres's pg wire protocol, so v0 skips it. Per [dolt#4843](https://github.com/dolthub/dolt/issues/4843), DDL doesn't honour autocommit anyway, making the explicit trailing `dolt_commit` the authoritative pattern. One commit per migration file is the resulting audit unit in `dolt_log` — cleaner than per-statement.

The two-step migrator pattern (`psql -f <file>` + `SELECT dolt_commit('-Am', '<name>')`) is documented at `nodes/poly/app/schema/README.md` for future contributors, along with the re-run / idempotency contract.

**Why `provision.sh` still exists (with reduced scope)**: after this PR, `infra/compose/runtime/doltgres-init/provision.sh` does only `CREATE DATABASE knowledge_{node}` and role creation (`knowledge_reader`, `knowledge_writer`). Schema DDL is removed from it — drizzle-kit owns schema. This eliminates the previous triple-source problem (Drizzle TS + `KNOWLEDGE_TABLE_DDL` constant + inline SQL in provision.sh) in favour of one source of truth.

**Previous justification struck**: earlier drafts of this task claimed "drizzle-kit's generated migrations would fight Dolt's model." That claim was factually wrong — Dolt's authoritative guidance is the opposite. See the Dolt blog and `@@dolt_transaction_commit` docs above.
...
- Per-node _schema_ divergence (poly's knowledge schema differing from operator's). Today every `knowledge_*` DB uses the same base `knowledge` table re-exported from `@cogni/node-template-knowledge`. When poly needs prediction-market-specific companion tables, they go in `nodes/poly/app/schema/knowledge.ts` next to the re-export — that's the whole point of node-local schema placement.
