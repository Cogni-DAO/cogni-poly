...
| `1890787d`     | swisstony           | `auto` (= `target_percentile_scaled`) p80 / $15 | `POLY_CANDIDATE_A_VALIDATION_*` | (just registered)        | candidate-a counterpart to the preview trust-twin; drives `/validate-candidate` exercises going forward | 🟡 ACTIVE — first non-orphan candidate-a row; replaces the 3 consolidation candidates once those soft-delete                                                |
