...
| `1890787d`     | swisstony           | `target_percentile_scaled` (via `auto`)                           | p80 / $15                       | `POLY_CANDIDATE_A_VALIDATION_*` | (just registered)        | 🟡 cand-a counterpart to the preview trust-twin; drives `/validate-candidate` exercises                                                       |
| `d66032aa`     | swisstony           | `target_percentile_scaled` (via `auto`) — slot for `position_gap` | p75 / $5 (default until PATCH)  | `POLY_CANDIDATE_A_GAP_*`        | (just registered)        | 🟡 PENDING — cand-a is on stale SHA `414d2439` (PR #93 head, pre-PR #92 contract); after re-flight from main, PATCH to `position_gap` p80/$15 |
...
| TENANT (short)          | TARGET WALLET | ALGO POLICY REF                         | ALGO POLICY CONFIG                                  | OWNER ENV-KEY               | DECISIONS / PLACED (24h) | DISPOSITION                                                                                                                       |
| ----------------------- | ------------- | --------------------------------------- | --------------------------------------------------- | --------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------- |
| `fb8f65d5` (trust-twin) | swisstony     | `target_percentile_scaled` (via `auto`) | p80 / $15                                           | `POLY_PREVIEW_TRUST_TWIN_*` | 8,580 / 43               | 🟡 **HARD LOCKED** — paper-mirror of derek's prod wallet on the legacy policy; THE control. Never PATCH this row's policy/config. |
| `376c594c` (swiss-gap)  | swisstony     | `position_gap`                          | p80 / $15 / `target_scale=1e-4` (bootstrap default) | `POLY_PREVIEW_GAP_*`        | (just registered)        | 🟡 first allowed preview experiment; the A/B counterpart to trust-twin for D2 phase-2 done condition                              |

### Gap

🟡 **Preview matrix now 2 rows** — control (trust-twin, tps) + experiment (swiss-gap, position_gap). Diff substrate is in place; needs ≥24h of activity before `paper-twin-diff.ts` produces a meaningful comparison.
🔴 **`position_gap` produced ZERO placements on candidate-a in 24h** (`f472b6ad`, RN1). Either RN1's positions are too small for `target_scale = 1e-4` (math: typical RN1 trade is 10–100 sh × 1e-4 = 0.001–0.01 desired sh; below any market floor) OR the planner's gap-negative skip is firing for a different reason. If the same shape repeats on preview's `swiss-gap` row (swisstony has much higher volume so this is less likely), it's a planner issue, not a per-target scale issue.
