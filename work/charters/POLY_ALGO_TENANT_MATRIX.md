...
| `13c81ec7` (swisstony-trust-twin) | swisstony     | `position_gap`                          | `capital_alloc_usdc=500000`                         | `POLY_PREVIEW_TENANT_SWISSTONY_TRUST_TWIN_*` | (just registered)        | 🟡 NEW — full-scale position_gap twin of swisstony. $500k copy budget, $1M paper capital. The "trust twin" for D2 phase-2 at prod volumes. |

### Gap

🟡 **Preview matrix now 3 rows** — control (trust-twin, tps), bootstrap-scale experiment (swiss-gap, position_gap @ default alloc), and full-scale experiment (swisstony-trust-twin, position_gap @ $500k alloc). Diff substrate is in place; needs ≥24h of activity before `paper-twin-diff.ts` produces a meaningful comparison across all three.
...
| `swisstony-trust-twin` | reuse `13c81ec7` (exists)                                                        | `position_gap` @ `mirror_capital_alloc_usdc=500000` ($1M paper cap) | full-scale experiment — tests D2 phase 2 against the control at production-volume swisstony book | placement volume comparable to trust-twin; delta to prod LIVE quantifies the gap-vs-tps policy choice at scale |
