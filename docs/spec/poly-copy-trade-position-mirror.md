...
- `FORWARD_ONLY_VIA_BASELINE` (task.5014, Phase 2.5) — `position_gap` reads `baseline_target_position_usdc` from `poly_copy_target_condition_baseline` per `(billing, target, condition)` and computes `delta = max(0, target_position_usdc_on_condition − baseline)`. No code path produces `desired_usdc > 0` without a persisted baseline row. The first post-activation fill captures the baseline (INSERT ON CONFLICT DO NOTHING) and the planner returns `before_baseline_snapshot` — preventing cold-start catch-up to pre-existing target positions at unfavorable prices. See docs/research/poly/range-relative-mirror-2026-05-26.md.
- `RANGE_DRIVES_DESIRED` (task.5014, Phase 2.5) — `desired_usdc = mirror_max_alloc_per_condition_usdc × min(delta / target_range_max_usdc, 1.0)`. No other path to `desired_usdc`. Anchors allocation to where target sits in their _assumed per-condition position range_ rather than to a moving Σ-book point. Per-condition cap replaces the per-target whole-book alloc shipped in the 2026-05-18 design.
- `NO_SELL_IN_MIRROR` (re-affirmed, task.5014) — `gap_shares ≤ 0 → skip followup_not_needed`. Target traders are long-only / hold-to-redemption (verified empirically — swisstony 0 SELLs in 90d, RN1 1 strategic burst tolerated as bounded cost). We never mirror a SELL.
...
- **2026-05-26 (Phase 2.5 — `position_gap` rewritten in place, task.5014):** task.5014 ships range-relative + forward-only baseline. The 2026-05-18 Σ-book scale + `mirror_capital_alloc_usdc` knob are deleted; in-place replacement on the same `kind: "position_gap"` discriminator. Anchors `desired_usdc` to where target sits in their _assumed per-condition range_ via `min(delta / target_range_max_usdc, 1.0) × mirror_max_alloc_per_condition_usdc`, where `delta` is measured against a per-(billing, target, condition) baseline captured at first post-activation observation. Forward-only by construction: pre-activation target positions never enter our objective. NO SELL re-affirmed (target traders are long-only / hold-to-redemption). New columns on `poly_copy_trade_targets`: `target_range_max_usdc`, `mirror_max_alloc_per_condition_usdc`, `mirror_activated_at`. New table: `poly_copy_target_condition_baseline` (PK = `(billing_account_id, target_id, condition_id)`, RLS-enabled). Dropped: `mirror_capital_alloc_usdc`, `getTargetTotalBookCost` adapter + its 30s cache. New invariants `FORWARD_ONLY_VIA_BASELINE` and `RANGE_DRIVES_DESIRED` above; `NO_SELL_IN_MIRROR` moved from open question to locked invariant. Tenant-matrix reset (deactivate the three live `position_gap` matrix tenants; POST fresh ones at `target_range_max_usdc=$10,000` and `mirror_max_alloc_per_condition_usdc ∈ {$5, $20, $200}`) is an OPERATOR ACTION via the existing copy-trade-targets API — not in the migration. CHECK grandfathers `disabled_at IS NOT NULL` rows so legacy rows remain valid post-migrate. Full design + parameterization in docs/research/poly/range-relative-mirror-2026-05-26.md and docs/research/poly/range-relative-parameterization-2026-05-26.md.
