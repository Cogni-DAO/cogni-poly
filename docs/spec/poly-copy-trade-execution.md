...
- **`MODE_DISCRIMINATOR_IN_ATTRIBUTES`** (`proj.poly-paper-trading`) — `intent.attributes.mode ∈ {"live", "paper"}` is the single source of truth for which execution backend handles an order. The trade-executor's `authorizedPlace` dispatches on this value, routing `"live"` to the `PolymarketClobAdapter` and `"paper"` to the `PaperAdapter` (sidecar IPC against `agent-next/polymarket-paper-trader`). Absent value falls back to `"live"` for back-compat with non-mirror callers. The bootstrap `PAPER_ENFORCE_MODE=paper` env override forces `"paper"` regardless of intent value — used on the `candidate-a` + `preview` overlays so flighted PRs and the continuous always-paper twin physically cannot place live orders. Cap accounting (`CAP_COUNTS_REALIZED_ON_CANCEL`) applies identically across modes: the paper sidecar reports realized `filled_size_usdc` on partial / full fills, and the ledger accumulates the same way for both. The shared `OrderIntent` port type does **not** gain a top-level `mode` field; `attributes` carries it as a passthrough record key.
