...
13. **Artifact provenance travels with the artifact** (bug.0321 Fix 4 → task.5006 `IMAGE_NATIVE_WITNESS`). Every `docker buildx build` in `scripts/ci/build-and-push-images.sh` bakes `org.opencontainers.image.revision=$BUILD_SHA` into the image config — the witness travels with the digest and is service-independent (node-apps, sidecars, migrators, scheduler-workers, future stdio-MCPs all carry it identically). `.promote-state/source-sha-by-app.json` on each deploy branch records **per-image** `source_sha` at promotion time (catalog image short-name keys: `poly`, `poly-paper-sidecar`, …; task.5006 retired the per-deploy-unit keying that produced PR #121's mixed-image lie). Production promotions copy the map forward from preview; `scripts/ci/verify-buildsha.sh` reads it and asserts `crane config <ref>@<digest>` returns `Labels["org.opencontainers.image.revision"] == map[image]` for every image of every promoted deploy unit — the only cross-PR-safe check when affected-only CI produces a mixed-SHA overlay, and the only role-agnostic check that covers sidecars/migrators/workers without per-image HTTP probes.
...
15. **Argo "Healthy" is necessary but not sufficient** (bug.0326). `status.health.status == Healthy` fires as soon as enough pods are Ready — including pods from the **old** ReplicaSet during a rolling update. `wait-for-argocd.sh` therefore requires the promoted app's own `Deployment` resource inside the Argo `Application` to report `status=Synced`, then asserts the new ReplicaSet has reached desired count (`status.updatedReplicas >= spec.replicas` AND `status.availableReplicas >= spec.replicas`). It does **not** wait for the old ReplicaSet to fully drain — that condition is not part of the contract and routinely false-fails when an old pod terminates slowly while the new RS is already serving traffic. `verify-buildsha.sh` (run after) is the canonical image-native-witness proof per Axiom 19 — it reads `org.opencontainers.image.revision` off the overlay-pinned digest via `crane config` and asserts equality with the per-image source-sha map.
16. **`CATALOG_IS_SSOT`** (task.0374; catalog v2 landed in PRs #61 + #70 — see [`catalog-v2.md`](./catalog-v2.md) for the full spec). `infra/catalog/*.yaml` is the single declaration site for nodes, node-shaped services, **and every image the repo ships** — one file = one deploy unit (Argo Application + per-env deploy-branch triple) that owns ≥1 `images:[]` (role: app / migrator / sidecar). Every consumer reads catalog: `scripts/ci/lib/image-tags.sh` exposes `DEPLOY_UNITS` / `DEPLOY_UNITS_WITH_NODE_APPS` / `ALL_IMAGES` + per-image lookups (`images_for_deploy_unit`, `dockerfile_for_image`, `image_name_for_image`, `image_tag_suffix_for_image`, `role_for_image`, `path_prefix_for_image`, `image_tag_for_image`, `public_url_for_deploy_unit`). `scripts/ci/detect-affected.sh` emits BOTH `targets_json` (image names — pr-build per-image matrix) and `deploy_units_json` (deduped deploy units — candidate-flight + promote-and-deploy per-branch matrices). `scripts/ci/promote-build-payload.sh` iterates `images_for_deploy_unit($NODE)`; `scripts/ci/promote-k8s-image.sh` is image-name-aware (rewrites the `images[]` entry whose `name:` matches; exit 0/1/2 contract — 2 = legitimate skip when overlay doesn't carry an image, e.g. paper-sidecar absent from production). Catalog conformance enforced on every PR by `pnpm check:catalog` (vitest invariants — schema shape + cross-file uniqueness + dockerfile-on-disk + exactly-one-role-app; blocking in `static` CI job). **Shape B (in-pod images — sidecars, migrators, stdio MCP)** lives as kustomize Components at `infra/k8s/components/sidecars/<image-name>/`; host overlays activate per env via one `components:` line. Adding a Shape B image = one entry in catalog `images:[]` + one Component dir + one `components:` line per overlay where active. **Classifier-extension follow-up:** `tests/ci-invariants/classify.ts` and `ci.yaml#single-node-scope` don't yet recognize `infra/catalog/<node>.yaml` or per-node overlays as node-domain; see [`node-ci-cd-contract.md § Node-autonomous service evolution`](./node-ci-cd-contract.md). **Out of scope (separate follow-ups):** `infra/compose/runtime/docker-compose.yml` per-service blocks; `infra/k8s/overlays/<env>/<node>/kustomization.yaml` `images:` block generation (still hand-maintained for human-set per-env activation toggles); Shape A re-exercise post-v2.
17. **`INFRA_K8S_MAIN_DERIVED` + `DEPLOY_BRANCH_OWNS_DIGESTS`** (bug.0334 structural rule; bug.5013 digest-authority carve-out, which subsumed the narrower bug.5012 placeholder-fill carve-out). Every file under `infra/k8s/` on a deploy branch is byte-identical to `main` at the promoted SHA, OR is the per-overlay `env-state.yaml` (the VM-truth file written by provision), OR is a digest pin in an overlay's `images:[]` block. The promote workflow does a two-pass rsync: (1) `--ignore-existing` seed pass for `env-state.yaml` (bootstraps new overlays without clobbering VM-written IPs); (2) `--delete --exclude='env-state.yaml'` authoritative sync for everything else. Image digests are mutated post-rsync by **two** sanctioned writers: (a) `restore-overlay-from-snapshot.sh` with `RESTORE_MODE=always` (both candidate-flight and promote-and-deploy since bug.5013) replays every prior digest pin from the deploy-branch's pre-rsync snapshot, and (b) `promote-k8s-image.sh` overwrites the subset of images this PR actually rebuilt. The deploy branch is therefore authoritative for live digest pins; `main:infra/k8s/overlays/<env>/<app>/kustomization.yaml` digest pins are only a cold-start seed (used the first time a new deploy unit / image lands, when the snapshot is empty). Restored digests originate from a prior promote's SHA, so they are not byte-identical to main's overlay at the current promoted SHA. Kustomize `replacements:` reads `env-state.yaml.data.VM_IP` and injects it into every EndpointSlice `/endpoints/0/addresses/0`, so VM IPs never live inline in `kustomization.yaml`. Violation: any non-digest, non-env-state diff between `main` and `deploy/<env>` after a promote. **bug.5013 evidence**: prior axiom ("main wins on real digests") regressed `poly-paper-sidecar` from `a72f751b` (PR #86) → `e96106e8` (PR #56 v0) at preview deploy commit `9f4e64855` when PR #88 (non-sidecar) merged; main's overlay pin had been frozen at v0 since PR #61 because the task.0349 digest-seed loop has produced zero `chore(preview)` commits on this repo's main.
...
19. **`PER_NODE_DEPLOY_SELF_VALIDATES`** (task.0376 + task.5006; positive form of the anti-pattern shared by bug.0321 / bug.0326 / bug.0328 / bug.0336 / bug.0344 / bug.0379 / bug.5016 PR #121 and the 2026-04-26 preview+production Alchemy-cap outage). A per-node matrix cell exits `success` only when **every image** it promoted carries `org.opencontainers.image.revision == map[image]`, read via `crane config` off the overlay-pinned digest — the image-native witness is the only signal that distinguishes "new code shipped" from "old code still running" from "lying-overlay promoted the wrong SHA". No cell may exit 0 on a hard mismatch. `TRANSITION_SAFE` carves out the cutover window: label-missing on a digest (image built pre-task.5006) and map-entry-missing for an image (next promote populates) are `::warning::` warn-skips, not failures — label-mismatch is always hard fail (the PR #121 lying-overlay class). A cell that promotes nothing for its node does not run verify; a cell that promotes anything MUST run verify (per Axiom 11). Argo `health.status == Healthy` and `/readyz 200` are explicitly **insufficient** as success signals: the former is satisfied by old-ReplicaSet pods during a rolling update (Axiom 15), and the latter is necessarily fragile to transient external dependencies (RPC providers, billing-cap rejections, datastore blips) and must therefore degrade non-fatally rather than drain pods. Aggregator `success` is the AND of per-cell `success`; a single skipped-with-promotions cell hard-fails the aggregator (Axiom 14, applied per-node). Enforced by `scripts/ci/resolve-cell-state.sh` (per-cell precondition gate), `scripts/ci/verify-buildsha.sh` (writes `verified-<node>.txt` per deploy unit when no image hard-mismatched and `MARKER_DIR` is set), and `scripts/ci/aggregate-decide-outcome.sh` (asserts every promoted cell has a matching verified marker). **Carve-out:** image-native witness proves the **promoted overlay digest** is honest; it does not prove pod-image-running parity. Argo + `wait-for-in-cluster-services` cover the rollout side. Runtime label-on-pod parity is a deferred follow-up.
...
- **contract probe passes**: every promoted image's baked `org.opencontainers.image.revision` OCI label matches the SHA that built its overlay digest (`scripts/ci/verify-buildsha.sh` reads the label via `crane config` and compares against the per-image `.promote-state/source-sha-by-app.json` on the deploy branch — task.5006 image-native witness, role-agnostic across apps / sidecars / migrators / workers)
...
- `verify-deploy` is job-level gated `if: promote-k8s.outputs.promoted_apps != ''` (Axiom 11). Runs `wait-for-argocd` → `wait-for-in-cluster-services` → `docker login ghcr.io` → `setup-crane` → `verify-buildsha` (image-native witness, task.5006). Depends on `deploy-infra` so secrets-restart completes before the contract probes run.
...
- `scripts/ci/update-source-sha-map.sh` — shared primitive that merges a single `image → sha` entry into the file. Untouched images retain their prior entry (merge, not overwrite).
- `scripts/ci/promote-build-payload.sh` — calls the primitive immediately after each successful per-image `promote-k8s-image.sh` call inside the same loop (`ATOMIC_OVERLAY_AND_MAP`, task.5006). Overlay digest + map entry land in the same upstream git commit; partial provenance states are structurally impossible.
- `.github/workflows/promote-and-deploy.yml` promote-k8s loop — same primitive, same per-image cadence.
...
**Reader**: `scripts/ci/verify-buildsha.sh` (task.5006). For each deploy unit in `NODES`, iterates `images_for_deploy_unit`; for each image reads the overlay digest from `OVERLAY_DIR/infra/k8s/overlays/$OVERLAY_ENV/<node>/kustomization.yaml`, the expected SHA from the map, and the baked `org.opencontainers.image.revision` OCI label via `crane config <ref>@<digest>`. Label-mismatch is hard fail; label-missing OR map-entry-missing is `::warning::` warn-skip (`TRANSITION_SAFE`). Images absent from the overlay (e.g. `poly-paper-sidecar` in production) are silently skipped.

**Why per-image keys**: PR #121 / run 26079941364 caught the prior per-deploy-unit shape lying — when a sidecar-only PR promoted, the map advanced `"poly": <sidecar-build-SHA>` even though the node-app's digest was unchanged. `verify-buildsha` then false-failed against the (correct) prior node-app SHA. Per-image keys + image-native label witness make this class of bug structurally impossible: the witness is on the digest, the digest is in the overlay, the map key matches the catalog image, and every check is local to one image.

## Deploy Branch Rules

- Deploy branches are long-lived, machine-written environment-state refs.
- `infra/k8s/` tracks `main` under invariant `INFRA_K8S_MAIN_DERIVED` (Axiom 17). Only image digests (mutated by `promote-k8s-image.sh` + `restore-overlay-from-snapshot.sh`) and `env-state.yaml` files (written by provision) are allowed to differ from main. All other overlay content — ConfigMap patches, Service patches, resource lists, `replacements:` blocks, `components:` lists — is rsynced from main every promote.
- **Preview digest seed on `main` (INFRA_K8S_MAIN_DERIVED, post-bug.5013)** — `promote-and-deploy.yml` rsyncs `infra/k8s/` from **main** at the promoted SHA into `deploy/preview` before mutating digest lines. After bug.5013, `main:infra/k8s/overlays/preview/<app>/kustomization.yaml` digest pins are **cold-start seeds only** — used the first time a new image lands, when the deploy-branch snapshot has no row for it. For every subsequent promote, the snapshot/restore sandwich replays the deploy branch's prior digest over main's pin, so an unaffected image cannot regress to whatever main happens to carry. The task.0349 seed loop (below) still exists as a best-effort refresh so `kustomize build` from `main` produces a usable manifest, but its output is no longer load-bearing for live deploys.
- **bug.5012 + bug.5013 — preview/production self-heal (snapshot wins for every digest)** — `promote-and-deploy.yml`'s promote-k8s matrix sandwiches the main-rsync step between a pre-rsync **snapshot** ([`scripts/ci/snapshot-overlay-digests.sh`](../../scripts/ci/snapshot-overlay-digests.sh)) and a post-rsync **restore** ([`scripts/ci/restore-overlay-from-snapshot.sh`](../../scripts/ci/restore-overlay-from-snapshot.sh) with `RESTORE_MODE=always`). The restore replays the deploy-branch's prior digest pin for **every** image (bug.5013 inversion of the prior `only-when-placeholder` mode). promote-build-payload then layers PR-affected digests on top, so the final overlay = (deploy-branch snapshot digests) ∪ (PR-rebuilt new digests). Closes two bug classes: (1) bug.5012 — a sidecar's main-overlay entry stayed on `<env>-placeholder-*` because no merged PR ever both built that image and triggered the [task.0349 seed](../../scripts/ci/promote-preview-seed-main.sh); placeholder reached Argo → ImagePullBackOff (live repro 2026-05-17 PR #83 → `poly-echo-sidecar:preview-placeholder-poly-echo-sidecar` stuck 7.5h). (2) bug.5013 — main's overlay carried a stale-but-real digest (the seed loop has produced zero `chore(preview)` commits on this repo) and the prior `only-when-placeholder` mode let it overwrite the snapshot's current digest, regressing unaffected sidecars on every non-sidecar PR's promote (live repro 2026-05-13 → 2026-05-18: `poly-paper-sidecar` frozen at PR #56's v0 digest while five subsequent sidecar PRs each promoted-then-immediately-regressed). Both candidate-flight (task.0373) and promote-and-deploy now use `RESTORE_MODE=always` — the rsync source is treated as advisory for digests in both cases, and the deploy branch is the authoritative live-digest source.
- **task.0349 — CI-owned seed (best-effort, no longer load-bearing post-bug.5013)** — Only when Flight Preview **dispatched** `promote-and-deploy` (preview lease was `unlocked`, not queue-only), [`promote-preview-digest-seed.yml`](../../.github/workflows/promote-preview-digest-seed.yml) runs [`scripts/ci/promote-preview-seed-main.sh`](../../scripts/ci/promote-preview-seed-main.sh) to refresh `main`'s overlay digest pins. Purpose post-bug.5013: keep `main` usable as a cold-start seed and as a target for `kustomize build` from clean checkouts. **Tri-state per image:** resolve `preview-{mergeSha}{suffix}` when the merge produced/retagged that image; else **retain** the existing pin from the overlay if it still resolves in GHCR; else **fail**. Emits at most **one** `chore(preview): …` commit to `main` per such merge when overlay lines change. **Queue-only** Flight Preview runs (lease `reviewing` / `dispatching`) upload outcome `queued` and **do not** trigger digest seed — no bot commits for those merges. The workflow uses a read-only job plus verified checkout (`head_sha` must be an ancestor of `origin/main`) before `contents:write` push. **`preview-dispatched-marker`** in `flight-preview.yml` is skipped (grey) when the flight was queue-only, so branch protection can require it as the "preview actually dispatched" signal. **Known bad on this repo:** zero `chore(preview)` commits have ever landed on cogni-poly main — root cause TBD (likely a missing `ACTIONS_AUTOMATION_BOT_PAT` secret, or the workflow_run trigger never firing for the merge_queue path). The bug.5013 fix (snapshot wins for every digest) makes this no longer a live-deploy correctness issue, but the seed should still be diagnosed and fixed so main remains a fresh cold-start seed — filed as a follow-up bug.
...
- **task.0373 + bug.5013 — restore-then-promote is universal** — Both `candidate-flight.yml` and `promote-and-deploy.yml` follow the same 4-stage chain on the deploy-branch checkout: (1) **snapshot pre-rsync** with [`scripts/ci/snapshot-overlay-digests.sh`](../../scripts/ci/snapshot-overlay-digests.sh) → 3-col TSV (`deploy_unit\timage_name\tref`), one row per `images:` entry per overlay (catalog v2 — Shape B sidecars included). (2) **rsync** overlay + base + Components from the source ref (PR branch for candidate-flight, main for promote-and-deploy). (3) **restore every digest pin** from snapshot via [`scripts/ci/restore-overlay-from-snapshot.sh`](../../scripts/ci/restore-overlay-from-snapshot.sh) with `RESTORE_MODE=always` (calls `promote-k8s-image.sh --no-commit --image-name` per row; tag-pins and `rc=2` "image removed from overlay" rows are no-ops). (4) **promote** PR-affected images via `promote-build-payload.sh` — naturally overwrites only the affected images, so unaffected siblings keep their snapshot digest. **Order matters**: restore runs **before** promote, not after. A "skip rows for units being promoted" filter is the bug — `PROMOTED_APPS` is per-deploy-unit while clobber happens per-image, so a partial-affected PR (e.g. paper-sidecar only on poly) would skip ALL of poly's snapshot rows and leave sibling sidecars on the post-rsync `<env>-placeholder-*` newTag → ImagePullBackOff. **Authority consequence:** the deploy branch is authoritative for digests in both lanes. `main:infra/k8s/overlays/<env>/<node>/kustomization.yaml` digest pins are **cold-start seeds only** — used the first time a new image lands when the snapshot is empty. Cold-start (first flight against a fresh `deploy/<env>-<node>`) has an empty snapshot → restore is a no-op, acceptable because no prior good state exists. The asymmetry between lanes is only in the rsync source (PR branch vs main); the digest-authority semantics are identical.
...
- [ ] **Production seed freshness**
      Preview overlay seeds on `main` are CI-owned (task.0349); candidate-flight self-heals `deploy/candidate-a` around the PR-branch rsync (task.0373) so `main:infra/k8s/overlays/candidate-a/**` digests are advisory only — no main-seed needed for candidate-a. Preview + production now share the placeholder-fill self-heal (bug.5012) so the main-overlay placeholder failure mode is closed for both envs. Still open: a detection signal for production seed staleness (Loki/Grafana query on overlay-digest age), and a CI-owned production seed equivalent of task.0349 (today a human dispatches `promote-and-deploy.yml` with `environment=production` to drive each production promote; the deploy-branch self-heal protects against placeholder regression but the main-overlay-stale signal is still operator vigilance).
