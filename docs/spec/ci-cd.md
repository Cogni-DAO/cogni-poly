...
16. **`CATALOG_IS_SSOT`** (task.0374; catalog v2 lands in cogni-poly PR #61, see [`catalog-v2.md`](./catalog-v2.md)). `infra/catalog/*.yaml` is the single declaration site for nodes, node-shaped services, **and every image the repo ships** — one file = one deploy unit (Argo Application + per-env deploy-branch triple) that owns ≥1 `images:[]` (role: app / migrator / sidecar). Every consumer reads catalog: `scripts/ci/lib/image-tags.sh` exposes `DEPLOY_UNITS` / `DEPLOY_UNITS_WITH_NODE_APPS` / `ALL_IMAGES` + per-image lookups (`images_for_deploy_unit`, `dockerfile_for_image`, `image_name_for_image`, `image_tag_suffix_for_image`, `role_for_image`, `path_prefix_for_image`, `image_tag_for_image`, `public_url_for_deploy_unit`). `scripts/ci/detect-affected.sh` emits BOTH `targets_json` (image names — pr-build per-image matrix) and `deploy_units_json` (deduped deploy units — candidate-flight + promote-and-deploy per-branch matrices). `scripts/ci/promote-build-payload.sh` iterates `images_for_deploy_unit($NODE)`; `scripts/ci/promote-k8s-image.sh` is image-name-aware (rewrites the `images[]` entry whose `name:` matches; exit 0/1/2 contract). Catalog conformance enforced on every PR by `pnpm check:catalog` (vitest invariants — schema shape + cross-file uniqueness + dockerfile-on-disk + exactly-one-role-app; blocking in `static` CI job). Adding an image = one entry in `images:[]`; zero script edits, zero workflow edits. **Out of scope of this axiom (separate follow-ups):** `infra/compose/runtime/docker-compose.yml` per-service blocks, and `infra/k8s/overlays/<env>/<node>/kustomization.yaml` `images:` block generation — both remain hand-maintained until a Kustomize-replacements / catalog-render pass lands. **Classifier-extension follow-up:** `tests/ci-invariants/classify.ts` and `ci.yaml#single-node-scope` don't yet recognize `infra/catalog/<node>.yaml` or per-node overlays as node-domain; see [`node-ci-cd-contract.md § Node-autonomous service evolution`](./node-ci-cd-contract.md).
