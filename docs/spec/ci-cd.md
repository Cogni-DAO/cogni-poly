...
17. **`INFRA_K8S_MAIN_DERIVED`** (bug.0334; bug.5012 placeholder-fill carve-out). Every file under `infra/k8s/` on a deploy branch is byte-identical to `main` at the promoted SHA, OR is the per-overlay `env-state.yaml` (the VM-truth file written by provision). The promote workflow does a two-pass rsync: (1) `--ignore-existing` seed pass for `env-state.yaml` (bootstraps new overlays without clobbering VM-written IPs); (2) `--delete --exclude='env-state.yaml'` authoritative sync for everything else. Image digests are mutated post-rsync by **two** sanctioned writers: (a) `promote-k8s-image.sh` writes affected-image digests for the current promote, and (b) `restore-overlay-from-snapshot.sh` with `RESTORE_MODE=only-when-placeholder` (preview/production) fills placeholder newTags from the deploy-branch's pre-rsync snapshot (per the bug.5012 self-heal bullet in Deploy Branch Rules) — restored digests originate from a prior promote's SHA, so they are not byte-identical to main's overlay at the current promoted SHA. Kustomize `replacements:` reads `env-state.yaml.data.VM_IP` and injects it into every EndpointSlice `/endpoints/0/addresses/0`, so VM IPs never live inline in `kustomization.yaml`. Violation: any non-digest, non-env-state diff between `main` and `deploy/<env>` after a promote.
...
- **Preview digest seed on `main` (INFRA_K8S_MAIN_DERIVED)** — `promote-and-deploy.yml` rsyncs `infra/k8s/` from **main** at the promoted SHA into `deploy/preview` before mutating digest lines. Therefore `main:infra/k8s/overlays/preview/<app>/kustomization.yaml` digest pins are the **rsync seed** for every later preview promote; they must stay pullable and aligned with GHCR. **Necessary but not sufficient** — when affected-only CI skips an image and that image's main-overlay entry is still a cold-start placeholder newTag, the rsync propagates the placeholder; the placeholder-fill chain below (bug.5012) is the safety net.
- **bug.5012 — preview/production self-heal (placeholder-fill restore-then-promote)** — `promote-and-deploy.yml`'s promote-k8s matrix sandwiches the main-rsync step between a pre-rsync **snapshot** ([`scripts/ci/snapshot-overlay-digests.sh`](../../scripts/ci/snapshot-overlay-digests.sh)) and a post-rsync **restore** ([`scripts/ci/restore-overlay-from-snapshot.sh`](../../scripts/ci/restore-overlay-from-snapshot.sh) with `RESTORE_MODE=only-when-placeholder`). The restore replays the deploy-branch's prior digest pin ONLY where main's overlay value is a tag pin (cold-start placeholder) — main's real digests are canonical per Axiom 17 and preserved. Closes the bug class where a sidecar's main-overlay entry stayed on `<env>-placeholder-*` because no merged PR ever both built that image and triggered the [task.0349 seed](../../scripts/ci/promote-preview-seed-main.sh); previously the placeholder reached Argo → ImagePullBackOff on every promote after the first cold-start (live repro 2026-05-17 PR #83 auto-flight-preview → `poly-echo-sidecar:preview-placeholder-poly-echo-sidecar` stuck 7.5h). The mode flag is the asymmetry vs candidate-flight's restore (task.0373 below): candidate-a uses `RESTORE_MODE=always` because its rsync source (PR-branch) is advisory; preview/production use `RESTORE_MODE=only-when-placeholder` because their rsync source (main) is canonical.
...
- **task.0373 — candidate-a self-heal (restore-then-promote, per-image)** — `candidate-flight.yml`'s `Sync ... to deploy branch` step rsyncs `infra/k8s/overlays/candidate-a/<node>/` from the **PR branch** (not from `main`). Stale PR-branch overlay digests would otherwise clobber known-good `deploy/candidate-a-<node>` digests and roll Argo to a regressing image. Fix is a 3-stage chain on the deploy-branch checkout: (1) **snapshot pre-rsync** with [`scripts/ci/snapshot-overlay-digests.sh`](../../scripts/ci/snapshot-overlay-digests.sh) → 3-col TSV (`deploy_unit\timage_name\tref`), one row per `images:` entry per overlay (catalog v2 — Shape B sidecars included). (2) **rsync** PR-branch overlay + base + Components. (3) **restore every digest pin** from snapshot via [`scripts/ci/restore-overlay-from-snapshot.sh`](../../scripts/ci/restore-overlay-from-snapshot.sh) (calls `promote-k8s-image.sh --no-commit --image-name` per row; tag-pins and `rc=2` "image removed from overlay" rows are no-ops). (4) **promote** PR-affected images via `promote-build-payload.sh` — naturally overwrites only the affected images, so unaffected siblings keep their snapshot digest. **Order matters**: restore runs **before** promote, not after. A "skip rows for units being promoted" filter is the bug — `PROMOTED_APPS` is per-deploy-unit while clobber happens per-image, so a partial-affected PR (e.g. paper-sidecar only on poly) would skip ALL of poly's snapshot rows and leave sibling sidecars on the post-rsync `<env>-placeholder-*` newTag → ImagePullBackOff. **Authority consequence:** because candidate-flight does not rsync from `main`, `main:infra/k8s/overlays/candidate-a/<node>/kustomization.yaml` digest pins have **no consumer** and are therefore **advisory and may lag** GHCR — explicitly the opposite of preview's `INFRA_K8S_MAIN_DERIVED` constraint. Cold-start (first flight against a fresh `deploy/candidate-a-<node>`) has an empty snapshot → restore is a no-op, acceptable because no prior good state exists. **No new workflow, no new privileged push to main, no new maintenance message-prefix.**
