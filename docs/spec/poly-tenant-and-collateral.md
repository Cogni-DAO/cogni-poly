...
| Mirror-pipeline cross-tenant enumerator                         | `app_service` (BYPASSRLS)                     | None — returns `(billing_account_id, created_by_user_id, target_wallet)` triples; per-tenant inner loop calls `orderLedger.forTenant(ctx)` (bug.5022). v0 carries an explicit `eq(billingAccountId, ...)` filter inside every query; **task.5012** Phase 1 wraps each `TenantOrderLedger` method in `withTenantScope(appDb, ctx.created_by_user_id, ...)` so RLS becomes the runtime DB-layer backstop. |
...
- **`ORDER_LEDGER_TENANT_CONTEXT_ENVELOPE`** _(bug.5022)_ — the order-ledger exposes a `forTenant(ctx: TenantContext)` factory; the canonical entry point for tenant-scoped reads + writes is the returned `TenantOrderLedger`. The envelope (`{billing_account_id, created_by_user_id}`) is constructed once at the top of each per-tenant inner loop from the cross-tenant enumerator's triples; the compiler steers callers away from tenant-anonymous reads. v0 cuts: (a) every SQL read inside the adapter — including the legacy root `snapshotState(target_id, billing_account_id)` — filters on both `billingAccountId` and `targetId` (fixing the pre-bug.5022 cross-tenant leak); (b) `.dependency-cruiser.cjs` forbids raw `polyCopyTradeFills` / `polyCopyTradeDecisions` imports outside `nodes/poly/app/src/features/trading/**`, with two temporary named exceptions owned by **task.5012**. The runtime DB-layer backstop (`withTenantScope(appDb, ctx.created_by_user_id, ...)` around every `TenantOrderLedger` method) is intentionally deferred to **task.5012** Phase 1 so the `serviceDb` → `appDb` role switch lands once across both the ledger and the privy adapter. Until then the explicit `billing_account_id` filter in every query is the structural floor. **task.5012** also extends the envelope across the remaining per-tenant poly repositories (Phase 2 migrates `privy-poly-trader-wallet.adapter.ts`; Phase 3 formalizes a `ResearchReadModelPort` for `wallet-analysis/copy-trade-pnl-service.ts`) and drops the two temporary lint-rule exceptions as it progresses. See `nodes/poly/app/src/features/trading/AGENTS.md` for the four code-level invariants this materializes: `TENANT_SCOPED_OPS_REQUIRE_CTX`, `TENANT_FILTER_IN_EVERY_SNAPSHOT_QUERY`, `RLS_BACKSTOP_TRACKED_IN_TASK_5012`, `CROSS_TENANT_OPS_NAMED_EXPLICITLY`.
