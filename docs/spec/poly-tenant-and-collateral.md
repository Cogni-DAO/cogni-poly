...
| Mirror-pipeline cross-tenant enumerator                         | `app_service` (BYPASSRLS)                     | None — returns `(billing_account_id, created_by_user_id, target_wallet)` triples; per-tenant inner loop calls `orderLedger.forTenant(ctx)` which opens a `withTenantScope(appDb, ctx.created_by_user_id, ...)` for every fills/decisions read AND write (bug.5022) |
...
- **`ORDER_LEDGER_TENANT_CONTEXT_ENVELOPE`** _(bug.5022)_ — the order-ledger exposes a `forTenant(ctx: TenantContext)` factory; tenant-scoped reads + writes are reachable only through the returned `TenantOrderLedger`. The envelope (`{billing_account_id, created_by_user_id, target_id}`) is constructed once at the top of each per-tenant inner loop from the cross-tenant enumerator's triples. Compile-time guarantee that no caller can ask for tenant-scoped state without naming the tenant. Implementation wraps every method in `withTenantScope(appDb, ctx.created_by_user_id, ...)` so RLS on `poly_copy_trade_{fills,decisions}` is the runtime backstop even if SQL drift drops a `.where()` clause. Pattern is the canonical shape for every future per-tenant repository in this codebase (see `nodes/poly/app/src/features/trading/AGENTS.md` for the four code-level invariants this materializes: `TENANT_SCOPED_OPS_REQUIRE_CTX`, `WITH_TENANT_SCOPE_WRAPS_EVERY_TENANT_READ`, `APP_DB_FOR_TENANT_DATA`, `EXPLICIT_BILLING_ACCOUNT_ID_DEFENSE_IN_DEPTH`).
