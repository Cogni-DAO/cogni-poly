...
| Mirror-pipeline cross-tenant enumerator                         | `app_service` (BYPASSRLS)                     | None — returns `(billing_account_id, created_by_user_id, target_wallet)` triples; per-tenant inner loop calls `orderLedger.forTenant(ctx)` (bug.5022). Every `TenantOrderLedger` read is wrapped in `withTenantScope(appDb, ctx.created_by_user_id, ...)` → Postgres RLS on `poly_copy_trade_{fills,decisions}` is the runtime DB-layer backstop. Writes (`insertPending`, `recordDecision`) migrate to the same wrap in **task.5012** Phase 1. |
...
- **`ORDER_LEDGER_TENANT_CONTEXT_ENVELOPE`** _(bug.5022)_ — the order-ledger exposes a `forTenant(ctx: TenantContext)` factory; the canonical entry point for tenant-scoped reads + writes is the returned `TenantOrderLedger`. The envelope (`{billing_account_id, created_by_user_id}`) is constructed once at the top of each per-tenant inner loop from the cross-tenant enumerator's triples; the compiler steers callers away from tenant-anonymous reads. v0 cuts: (a) every SQL read inside the adapter — including the legacy root `snapshotState(target_id, billing_account_id)` — filters on both `billingAccountId` and `targetId` (fixing the pre-bug.5022 cross-tenant leak); (b) every read on the `TenantOrderLedger` (`snapshotState`, `cumulativeIntentForMarketToken`, `hasOpenForMarket`, `findOpenForMarket`) runs inside `withTenantScope(appDb, ctx.created_by_user_id, ...)`, activating Postgres RLS on `poly_copy_trade_{fills,decisions}` as the runtime DB-layer backstop — even if a future query forgets the explicit filter, the DB strips rows owned by another user. The two `TenantOrderLedger` writes (`insertPending`, `recordDecision`) still flow through `serviceDb` and stamp tenant attribution explicitly in the row values — not the leak surface; **task.5012** Phase 1 migrates them onto the same `withTenantScope` wrap. **task.5012** also extends the envelope across the remaining per-tenant poly repositories (Phase 2 migrates `privy-poly-trader-wallet.adapter.ts`; Phase 3 formalizes a `ResearchReadModelPort` for `wallet-analysis/copy-trade-pnl-service.ts`). See `nodes/poly/app/src/features/trading/AGENTS.md` for the four code-level invariants this materializes: `TENANT_SCOPED_OPS_REQUIRE_CTX`, `TENANT_FILTER_IN_EVERY_SNAPSHOT_QUERY`, `FORTENANT_READS_THROUGH_RLS`, `CROSS_TENANT_OPS_NAMED_EXPLICITLY`.
